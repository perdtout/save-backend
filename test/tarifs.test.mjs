...
const rows = {
  tarifs: [
    ...
    ["Apple", "iPhone 15", 59.99, "", "", "Bolt", 50, 30, "", "", "", 40, 30, "", "", "", "", "Oui", ""],        // vitre partout
...
check(i14?.reparations[2].magasins.join() === "Pontarlier", "vitre Apple listée en Prix fixes → Pontarlier uniquement");
check(i14?.reparations[0].magasins.length === 0, "écran → tous les magasins");
const v15 = trouver("Apple", "iPhone 15")?.reparations.find(r => r.id === "vitre-ar");
check(v15?.prix === 99.99 && v15?.magasins.length === 0, "vitre Apple absente de Prix fixes → tous les magasins");
