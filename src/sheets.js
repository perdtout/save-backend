...
// Diagnostic temporaire (09/09/2026) : à retirer une fois le souci des titres MVP résolu.
console.log(`🔎 buildGoatData: ${rows.length} lignes reçues, ${titlesHistory.length} titres MVP détectés (types: ${[...new Set(rows.map(r => r.periodType))].join(", ")})`);
if (rows.length) console.log(`🔎 Exemple ligne brute : date=${JSON.stringify(rows[0].date)} (${typeof rows[0].date}), periodType=${JSON.stringify(rows[0].periodType)}, mvp=${rows[0].mvp}`);
